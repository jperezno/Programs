...
#Purify protocol is a self made protocol that aims to only call the instructions of the Purify.py example
#this class is based in the same class used in purify.py: class FilteringExample(LocalProtocol):
class PurifyProtocol(LocalProtocol):
    ...
    def __init__(self, node_left, node_right, num_runs, epsilon=0.3):
        ...
        self.add_subprotocol(Filter(node_left, node_left.ports["ccon_R"],
                                    start_expression=None, epsilon=epsilon, name="purify_L"))
        self.add_subprotocol(Filter(node_right, node_right.ports["ccon_L"],
                                    start_expression=None, epsilon=epsilon, name="purify_R"))
        print("Debug: Added subprotocols:", list(self.subprotocols.keys()))

        #Here we make it so purify_A=purify_L and same for B=R
        ...
        ####This part makes sure and prints which nodes are being used/connected, original program brakes here
    ...
    while len(nodes_copy)>2: #instructions until nodes is only two
        ...
        for i in range(len(nodes_copy)):
            if (i % 2)==1:
                ...
                print(f"Node_2 ccon_L connected to: {nodes_copy[i+1].ports['ccon_L'].connected_port}")
