...
#Verify once we distil one of pairs has to disapear, only one remains
#is distil pair being stored in memory?
#we can't be sure code works 
def example_network_setup(source_delay=1e5, source_fidelity_sq=0.7,
                          node_distance=1):
...
def example_sim_setup(node_a, node_b, num_runs):
    ...
    """
    filt_example = FilteringExample(node_a, node_b, num_runs=num_runs)
...
if __name__ == "__main__":
    ...
    filt_example.start()
    ns.sim_run()
    print("Average fidelity of generated entanglement with distilation: {}".format(
