...
.. aafig::
    ...
    |                     | +------------------------------------+ |                     |                      |
    |                     | |                                    | |                     |                      |
    |                     O-* "Connection: ClassicalChannelsAB <->"*-O                     |- Classical Channel- 0|
    |                     | |                                    | |                     |
    |                     | +------------------------------------+ |                     |
    +---------------------+                                        +---------------------+
...
def example_network_setup(source_delay=1e5, source_fidelity_sq=0.8, depolar_rate=1000,
    ...
    qchannelBC = QuantumChannel("QChannel_B->C", length=node_distance,
                              models={"quantum_loss_model": None,
                                      "delay_model": FibreDelayModel(c=200e3)},
                              depolar_rate=0)
    port_name_c, port_name_b_right = network.add_connection(
        node_c, node_b, channel_to=qchannelBC, label="quantum")
    #Linking ports
    # Link Alice ports:
    ...
    #Link Charlie Ports
    node_c.subcomponents["QSource_C"].ports["qout1"].forward_output(
        node_c.ports[port_name_c])
    node_c.subcomponents["QSource_C"].ports["qout0"].connect(
        node_c.qmemory.ports["qin0"])
    # Link Bob ports:
